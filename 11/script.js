...
const createAddStudentForm = (students) => {
  ...
  const tuitionStartedInput = createInput(
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (
      ...
      const newStudent = await addStudent({
        ...
        tuitionEnds: (
          new Date(Number(tuitionStartedInput.value.trim().slice(0, 4)) + 4 + tuitionStartedInput.value.trim().slice(4))
        ).toLocaleDateString(),
      });
  ...
  wrapper.addEventListener("input", () => {
    ...
    timeout = setTimeout(() => {
      const filteredStudents = students.filter((student) => {
        return (
          `${student.secondName} ${student.firstName} ${student.thirdName}`.toLowerCase().includes(fullNameInput.value.toLowerCase()) &&
...
const createStudentsTable = (students) => {
  ...
  tableHeader.addEventListener("click", (e) => {
    if (e.target.textContent === "Имя") {
      ...
      const sortedStudents = students.sort((a, b) =>
        new Date(`${a.birth.slice(6)}-${a.birth.slice(3, 5)}-${a.birth.slice(0, 2)}`) >
        new Date(`${b.birth.slice(6)}-${b.birth.slice(3, 5)}-${b.birth.slice(0, 2)}`) ? 1 : -1
      );
      rerenderTableBody(sortedStudents);
