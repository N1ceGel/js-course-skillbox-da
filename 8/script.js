...
const createTodoApp = (container, title = "Список дел", key) => {
  ...
  // Делегирование событий для кнопок "Готово" и "Удалить"
  todoList.addEventListener("click", (e) => {
    ...
    if (target.classList.contains("btn-success")) {
      ...
      if (listItem) {
        ...
        const todoObjectItem = todos.find((todo) => todo.id === Number(listItem.dataset.id));
        todoObjectItem.done = !todoObjectItem.done;
        setItemsToLocalStorage(key, todos);
      }
      ...
      if (listItem) {
        listItem.remove();
        todos = todos.filter((todo) => todo.id !== Number(listItem.dataset.id));
        setItemsToLocalStorage(key, todos);
      }
    }
  });
